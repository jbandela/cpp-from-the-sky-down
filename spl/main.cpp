...
// Handle complete -> incremental conversions
    ...
    struct stage_impl<Derived<StageProperties, Parameters...>> {
        ...
        using child = Derived<StageProperties, Parameters...>;
        ...
        constexpr static auto input_processing_style = StageProperties::input_processing_style;
    ...
    public:
        ...
        constexpr auto make(Next &&next) {
            if constexpr (std::is_same_v<Parameters, void>) {
                return StageImpl<StageProperties, TypeParameters...>{std::forward<Next>(next)};
            } else {
                return StageImpl<StageProperties, TypeParameters...>{std::forward<Next>(next),
    ...
    constexpr auto compose(Ts &&... ts) {
        return composed{[...args = std::forward<Ts>(ts)](auto&& prev, auto&&... next)constexpr mutable{
            if constexpr (sizeof...(next) == 1) {
                return (std::forward<decltype(prev)>(prev) + ... + std::move(args)).make(
        ...
        struct input_factory {
            ...
            constexpr auto make(Next &&next) {
                if constexpr (std::is_invocable_v<Factory, decltype(previous), decltype(next)>) {
                    ...
                    return previous.make(factory.template make<stage_properties<Input,
                            Next,
                            previous_output_processing_style,
                            input_processing_style,
                            output_processing_style>>(
    ...
    struct accumulate_in_place_impl
        ...
        constexpr accumulate_in_place_impl(auto &&stage_properties,
                ...
                : base(std::forward<decltype(stage_properties)>(stage_properties)),
                  accumulated(std::get<0>(tuple)),
                  f(std::get<1>(tuple)) {}

        constexpr accumulate_in_place_impl(auto &&stage_properties, F &&f)
                : base(std::forward<decltype(stage_properties)>(stage_properties)), accumulated{},
                  f(std::move(f)) {}
    ...
    template<typename StageProperties, typename Predicate>
    struct filter_impl : stage_impl<filter_impl<StageProperties,
            Predicate>> {
